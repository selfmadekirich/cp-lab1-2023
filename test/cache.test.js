...
test('can set and get not-empty key,value,hit', () => {
    const c = new Cache()
    result = c.setValue("validKey",123,10)
...
test('can not set value with array key', () => {
    const c = new Cache()
    result = c.setValue([1,2,3,4],1)
    expect(result).toBe(false);
});

test('can not set value with  dict-key', () => {
    const c = new Cache()
    result = c.setValue({"key":"value1"},1)
    expect(result).toBe(false);
});
...
test('can not set value with negative hit', () => {
    const c = new Cache()
    result = c.setValue("validKey",-100)
    expect(result).toBe(false);
});

test('can not set value with zero hit', () => {
    const c = new Cache()
    result = c.setValue("validKey",0)
    expect(result).toBe(false);
});

test('can set and get string value', () => {
    const c = new Cache()
    result = c.setValue("validKey","string")
    expect(result).toBe(true);
...
test('can set and get array value', () => {
    const c = new Cache()
    result = c.setValue("validKey",[1,2,3,4,5])
    expect(result).toBe(true);
    expect(c.getValue("validKey")).toBe([1,2,3,4,5]);
});

test('can set and get dict value', () => {
    const c = new Cache()
    result = c.setValue("validKey",{"key1":"value1","key2":"value2"})
    expect(result).toBe(true);
    expect(c.getValue("validKey")).toBe({"key1":"value1","key2":"value2"});
});


test('can set and get value with integer key', () => {
    const c = new Cache()
    result = c.setValue(123,"123")
    expect(result).toBe(true);
...
test('getStatByKey returns actual stats for key', () => {
    const c = new Cache()
    result = c.setValue(123,"123",1203)
    expect(result).toBe(true);
    expect(c.getStatByKey(123)).toBe({"key":123,"value":"123","hit remains":1203});
});

test('every getValue dicreases hit number', () => {
    const c = new Cache()
    result = c.setValue(123,"123",2)
    expect(result).toBe(true);
    expect(c.getValue(123)).toBe("123");
    expect(c.getStatByKey(123)).toBe({"key":123,"value":"123","hit remains":1});
});
